...
async fn test_storage_engine_initialization() {
    ...
    assert!(
        storage.is_ok(),
        "Storage engine should initialize successfully"
    );
...
async fn test_insert_and_select_rows() {
    ...
    let id1 = storage
        .insert_row("test_table", row1.clone())
        .await
        .unwrap();
    let id2 = storage
        .insert_row("test_table", row2.clone())
        .await
        .unwrap();
...
async fn test_dna_compression_integration() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully compress and store large data"
    );
...
async fn test_schema_validation() {
    ...
    assert!(
        result.is_err(),
        "Should reject row with missing required field"
    );
}
