...
use crate::dna::{
    QuantumDNACompressor, DNACompressor, DNACompressionConfig, DNABase, DNAError,
    CompressedDNA, DNASequence
};
...
mod unit_tests {
    ...
    async fn test_different_error_correction_strengths() {
        ...
        for strength in [1, 16, 32, 64, 128] {
    ...
    async fn test_compression_metadata() {
        ...
        assert_eq!(compressed.sequence.metadata.error_correction_strength, 
...
mod property_tests {
    ...
    proptest! {
        ...
        fn test_compression_ratio_bounds(data in prop::collection::vec(any::<u8>(), 1..1000)) {
            ...
            prop_assert!(ratio >= 0.1 && ratio <= 10.0, "Compression ratio {} out of bounds", ratio);
        }
...
mod error_tests {
    ...
    async fn test_corrupted_base_sequence() {
        ...
        compressed.sequence.bases.truncate(compressed.sequence.bases.len() / 2);
