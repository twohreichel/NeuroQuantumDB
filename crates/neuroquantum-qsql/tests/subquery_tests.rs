...
async fn setup_test_tables(storage_arc: Arc<tokio::sync::RwLock<StorageEngine>>) {
    ...
    storage_guard.create_table(departments_schema).await.unwrap();
    ...
    for (id, name, active) in departments.iter() {
        ...
        row.fields.insert("active".to_string(), Value::Boolean(*active));
    ...
    for (id, name, age, dept_id) in users.iter() {
        ...
        match dept_id {
            Some(d) => row.fields.insert("department_id".to_string(), Value::Integer(*d)),
    ...
    for (id, user_id, amount) in orders.iter() {
        ...
        row.fields.insert("id".to_string(), Value::Integer(*id));
        row.fields.insert("user_id".to_string(), Value::Integer(*user_id));
...
async fn create_test_executor() -> (TempDir, Arc<tokio::sync::RwLock<StorageEngine>>, QueryExecutor) {
...
mod parser_tests {
    ...
    fn test_parse_scalar_subquery_in_select_list() {
        ...
        assert!(result.is_ok(), "Failed to parse scalar subquery in SELECT list: {:?}", result.err());

        let stmt = result.unwrap();
        if let Statement::Select(select) = stmt {
            assert_eq!(select.select_list.len(), 2);
            // First item should be 'name' identifier
            if let SelectItem::Expression { expr: Expression::Identifier(name), .. } = &select.select_list[0] {
                assert_eq!(name, "name");
            } else {
                panic!("Expected first SELECT item to be identifier 'name'");
            }
            // Second item should be a scalar subquery with alias 'avg_age'
            if let SelectItem::Expression { expr: Expression::ScalarSubquery { .. }, alias } = &select.select_list[1] {
    ...
    fn test_parse_scalar_subquery_in_where() {
        ...
        let result = parser.parse(sql);
        assert!(result.is_ok(), "Failed to parse scalar subquery in WHERE: {:?}", result.err());

        let stmt = result.unwrap();
        if let Statement::Select(select) = stmt {
            assert!(select.where_clause.is_some());
            // The WHERE clause should contain a BinaryOp with ScalarSubquery on the right
            if let Some(Expression::BinaryOp { right, operator, .. }) = &select.where_clause {
    ...
    fn test_parse_in_subquery() {
        ...
        let result = parser.parse(sql);
        assert!(result.is_ok(), "Failed to parse IN subquery: {:?}", result.err());
    ...
    fn test_parse_not_in_subquery() {
        ...
        let result = parser.parse(sql);
        assert!(result.is_ok(), "Failed to parse NOT IN subquery: {:?}", result.err());
    ...
    fn test_parse_exists_subquery() {
        ...
        if let Statement::Select(select) = stmt {
            ...
            if let Some(Expression::Exists { negated, .. }) = &select.where_clause {
                ...
                panic!("Expected Exists in WHERE clause, got: {:?}", select.where_clause);
            }
        } else {
            panic!("Expected SELECT statement");
        }
    ...
    fn test_parse_not_exists_subquery() {
        ...
        let result = parser.parse(sql);
        assert!(result.is_ok(), "Failed to parse NOT EXISTS subquery: {:?}", result.err());
    ...
    fn test_parse_derived_table() {
        ...
        let result = parser.parse(sql);
        assert!(result.is_ok(), "Failed to parse derived table: {:?}", result.err());
...
mod execution_tests {
    ...
    async fn test_execute_in_subquery() {
        ...
        assert!(check_result.is_ok(), "Check query failed: {:?}", check_result.err());
        let check_res = check_result.unwrap();
        println!("Departments table has {} rows: {:?}", check_res.rows_affected, check_res.rows);
        assert!(check_res.rows_affected >= 3, "Expected at least 3 departments");
        ...
        assert!(subquery_result.is_ok(), "Subquery alone failed: {:?}", subquery_result.err());
        let subquery_res = subquery_result.unwrap();
        println!("Subquery (no WHERE) returned {} rows: {:?}", subquery_res.rows_affected, subquery_res.rows);
        assert!(subquery_res.rows_affected >= 3, "Expected at least 3 departments");
        ...
        assert!(users_result.is_ok(), "Users query failed: {:?}", users_result.err());
        let users_res = users_result.unwrap();
        println!("Users table has {} rows: {:?}", users_res.rows_affected, users_res.rows);
        ...
        assert!(in_list_result.is_ok(), "IN list query failed: {:?}", in_list_result.err());
        let in_list_res = in_list_result.unwrap();
        println!("IN list returned {} rows: {:?}", in_list_res.rows_affected, in_list_res.rows);
        assert_eq!(in_list_res.rows_affected, 4, "Expected 4 users in departments 1 and 2");
        ...
        assert!(result.is_ok(), "Failed to execute IN subquery: {:?}", result.err());
        let query_result = result.unwrap();
        println!("IN subquery returned {} rows: {:?}", query_result.rows_affected, query_result.rows);

        // Should return at least some users (since at least 5 users have departments)
        assert!(query_result.rows_affected >= 4, "Expected at least 4 users in departments");
    }
    ...
    async fn test_execute_derived_table() {
        ...
        assert!(result.is_ok(), "Failed to execute derived table: {:?}", result.err());
        let query_result = result.unwrap();

        // Should return Charlie (30), Diana (35), Eve (40), Frank (45)
        assert_eq!(query_result.rows_affected, 4, "Expected 4 users older than 25");
    }
}
