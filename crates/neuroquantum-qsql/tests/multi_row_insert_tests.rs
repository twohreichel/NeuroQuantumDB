...
fn test_parse_multi_row_insert() {
    ...
    assert!(result.is_ok(), "Failed to parse multi-row INSERT: {:?}", result.err());
...
async fn test_execute_multi_row_insert() {
    ...
    assert_eq!(rows[0].fields.get("name"), Some(&Value::Text("Alice".to_string())));
    assert_eq!(rows[0].fields.get("email"), Some(&Value::Text("alice@example.com".to_string())));
    assert_eq!(rows[0].fields.get("age"), Some(&Value::Integer(25)));
    
    assert_eq!(rows[1].fields.get("name"), Some(&Value::Text("Bob".to_string())));
    assert_eq!(rows[1].fields.get("email"), Some(&Value::Text("bob@example.com".to_string())));
    assert_eq!(rows[1].fields.get("age"), Some(&Value::Integer(30)));
    
    assert_eq!(rows[2].fields.get("name"), Some(&Value::Text("Charlie".to_string())));
    assert_eq!(rows[2].fields.get("email"), Some(&Value::Text("charlie@example.com".to_string())));
...
async fn test_large_batch_insert() {
    ...
    let sql = format!("INSERT INTO logs (message) VALUES {}", values_parts.join(", "));
