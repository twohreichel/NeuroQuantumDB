...
async fn test_hash_join_left_join() {
    ...
    assert_eq!(charlie_row.get("orders.order_id"), Some(&QueryValue::Null));
}
...
async fn test_hash_join_right_join() {
    ...
    assert_eq!(orphan_order.get("users.name"), Some(&QueryValue::Null));
}
...
async fn test_hash_join_full_outer_join() {
    ...
    let has_null_user = result
        .rows
        .iter()
        .any(|row| matches!(row.get("users.name"), Some(&QueryValue::Null)));
    let has_null_order = result
        .rows
        .iter()
        .any(|row| matches!(row.get("orders.order_id"), Some(&QueryValue::Null)));

    assert!(has_null_user, "Should have at least one row with NULL user");
    assert!(
        has_null_order,
        "Should have at least one row with NULL order"
    );
}
...
async fn test_nested_loop_join_for_small_tables() {
    ...
    assert!(result
        .rows
        .iter()
        .all(|row| { row.contains_key("name") && row.contains_key("amount") }));
}
