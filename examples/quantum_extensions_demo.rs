...
use neuroquantum_core::quantum::qubo::{QUBOSolver, QUBOConfig};
use neuroquantum_core::quantum::tfim::{TFIMSolver, TransverseFieldConfig, FieldSchedule};
use neuroquantum_core::quantum::parallel_tempering::{
    ParallelTempering, ParallelTemperingConfig, TemperatureDistribution, ising_energy_function,
};
...
fn demo_qubo_tsp() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Cities: 5");
    println!("Distance matrix size: {}x{}", dist_matrix.nrows(), dist_matrix.ncols());
...
fn demo_tfim_ferromagnetic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Energy: {:.4}", solution.energy);
    println!("Ground state probability: {:.6}", solution.ground_state_prob);
...
async fn demo_parallel_tempering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let couplings = DMatrix::from_fn(10, 10, |i, j| {
        if i != j {
            ((i as f64 - j as f64).abs() as f64).recip()
        } else {
            0.0
        }
    });
    ...
    println!("Temperature range: {:.1} - {:.1}", config.min_temperature, config.max_temperature);
    ...
    println!("Exchange acceptance rate: {:.2}%", solution.acceptance_rate * 100.0);
    println!("Total exchanges: {} ({} accepted)", solution.total_exchanges, solution.accepted_exchanges);
    println!("Time: {:.2}ms", solution.computation_time_ms);
...
async fn demo_method_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Energy: {:.4}, Quality: {:.1}%, Time: {:.2}ms",
        sol1.energy, sol1.quality * 100.0, sol1.computation_time_ms);
    ...
    println!("   Energy: {:.4}, Quality: {:.1}%, Time: {:.2}ms",
        sol2.energy, sol2.quality * 100.0, sol2.computation_time_ms);
    ...
    println!("   Energy: {:.4}, Tunneling: {}, Time: {:.2}ms",
    ...
    println!("   Energy: {:.4}, Acceptance: {:.1}%, Time: {:.2}ms",
        sol4.best_energy, sol4.acceptance_rate * 100.0, sol4.computation_time_ms);

    println!("\n📈 Summary:");
    println!("   Best energy: {:.4} (Method {})",
