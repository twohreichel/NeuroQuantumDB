...
async fn main() -> anyhow::Result<()> {
    ...
    println!("🎉 Alle Tests erfolgreich abgeschlossen in {:?}!", total_time);
    println!("\n📊 Test Summary:");
...
async fn run_iot_demo() -> anyhow::Result<()> {
    ...
    println!("   📡 Generiert: {} IoT Sensordaten aus 5 deutschen Städten", iot_data.len());

    // Zeige Beispieldaten
    let sample = &iot_data[0];
    println!("   📍 Beispiel Sensor: {} in Berlin", sample.sensor_id);
    println!("   🌡️  Temperatur: {:.1}°C, Luftfeuchtigkeit: {:.1}%",
             sample.temperature, sample.humidity);
    println!("   🔋 Batterie: {}%, Signal: {}dBm",
    ...
    println!("   🧬 DNA Kompression: {}B → {}B (Ratio: 4:1)", original_size, compressed_size);

    // Simuliere Quantum Search
    let search_start = Instant::now();
    let critical_sensors = iot_data.iter()
        .filter(|s| s.temperature > 30.0 && s.battery_level < 20)
        .count();
    let search_time = search_start.elapsed();

    println!("   🔍 Quantum Search: {} kritische Sensoren in {:?}", critical_sensors, search_time);
...
async fn run_medical_demo() -> anyhow::Result<()> {
    ...
    println!("   💓 Vitalwerte: {}bpm, {}/{}mmHg, {:.1}°C",
    ...
    println!("   🧠 EEG Daten: {} Messpunkte, {} neurale Muster",
    ...
    println!("   🧬 Neuromorphic Learning: {} ähnliche Muster in {:?}", pattern_matches, learning_time);
...
async fn run_finance_demo() -> anyhow::Result<()> {
    ...
    println!("   📈 Generiert: {} Finanzmarkt-Datensätze", financial_data.len());

    let sample = &financial_data[0];
    println!("   💹 Symbol: {}, Preis: ${:.2}", sample.symbol, sample.price);
    println!("   📊 OHLC: ${:.2}/{:.2}/{:.2}/{:.2}",
             sample.market_data.open, sample.market_data.high,
             sample.market_data.low, sample.market_data.close);
    println!("   📰 Sentiment: News {:.2}, Social {:.2}",
             sample.sentiment_analysis.news_sentiment,
             sample.sentiment_analysis.social_sentiment);

    // Simuliere Quantum Portfolio Optimization
    let symbols = ["AAPL", "GOOGL", "MSFT", "TSLA"];
    let optimization_start = Instant::now();
    let optimal_assets = financial_data.iter()
        .filter(|d| symbols.contains(&d.symbol.as_str()) && d.quantum_indicators.quantum_momentum > 0.0)
        .count();
    let optimization_time = optimization_start.elapsed();

    println!("   ⚛️  Quantum Portfolio: {} optimale Assets in {:?}", optimal_assets, optimization_time);
...
async fn run_qsql_demo() -> anyhow::Result<()> {
    ...
    println!("   📝 QSQL Test Queries: {} verschiedene Syntax-Features", queries.len());
...
async fn run_performance_demo() -> anyhow::Result<()> {
    ...
    println!("   🔍 Query Latency: {}ms (Quantum optimiert)", query_latency);
